...
async function run() {
  ...
  const thread = await send("thread/start", { approvalPolicy: "auto", sandbox: "none", cwd: "/tmp/test-ws" });
  const threadId = thread.result?.thread?.id;
  console.log("[thread]", threadId);
  
  const turn = await send("turn/start", {
    threadId,
    input: [{ type: "text", text: "Create a file called hello.txt with the content 'Hello from Symphony'" }],
    cwd: "/tmp/test-ws",
    title: "CAP-32: Test Hello World"
  });
  console.log("[turn raw]", JSON.stringify(turn, null, 2));
